...
function setup(){
    canvas = createCanvas(windowWidth*70/100, windowHeight);
...
class CIEFileReader{
    ...
    userFileLoaded(file){
        ...
        if(file.type === 'text'){
            ...
            fileString.forEach(element => {
                let lineExcluded = /^[0-9,\s|.]/;
                let line = lineExcluded.test(element);
                if(line){
                    let row1 = element.replace('\t', ',');
                    let row = row1.split(',');
    ...
    calculexy(){
        ...
        inputx.value(round(x, 2));
        inputy.value(round(y, 2));
    ...
    addCIEpoint(){
        ...
        let file_id = input_file_id.value();
            xcoordinate.push(round(x1, 2));
            ycoordinate.push(round(y1, 2));
    ...
    CIEpoint(){
        ...
        for(let j = 0; j <= xcoordinate.length-1; j++){
            push();
            translate(100, 462);
            text(`${symbolpoint[j]}`, float(xcoordinate[j])*378, float(ycoordinate[j])*-378);
            pop();
            push();
            translate(0, 100);
    ...
    saveImage(){
        resizeCanvas(475, 545);
        save(canvas, 'CIE1931chromaticitydiagram.jpg');
        setup();
    }
}
